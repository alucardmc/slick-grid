(function ($) {
  ...
  function CellRangeSelector(options) {
    ...
    function handleDrag(e, dd) {
        ...
        e.stopImmediatePropagation();
        if(!_$activeCanvas) init_values();
        var end = _grid.getCellFromPoint(
            e.pageX - _$activeCanvas.offset().left + _columnOffset,
            e.pageY - _$activeCanvas.offset().top + _rowOffset
        );
        if ( (!_grid.canCellBeSelected( end.row, end.cell ) ) 
             || ( !_isRightCanvas && _gridOptions.frozenColumn > 0 && ( end.cell > _gridOptions.frozenColumn ) )
             || ( _isRightCanvas && _gridOptions.frozenColumn > 0 && ( end.cell <= _gridOptions.frozenColumn ) )
             || ( !_isBottomCanvas && _gridOptions.frozenRow > 0 && ( end.row >= _gridOptions.frozenRow ) )
             || ( _isBottomCanvas && _gridOptions.frozenRow > 0 && ( end.row < _gridOptions.frozenRow ) )
           ) {
            return;
        }
