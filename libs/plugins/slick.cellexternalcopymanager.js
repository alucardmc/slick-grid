(function ($) {
  ...
  function CellExternalCopyManager(options) {
    ...
    */
    ...
    function _decodeTabularData(_grid, ta, _on_self){
      ...
      //TODO: вставка новых строк в случае когда данные скопипаста не помещаются
      ...
      var clipCommand = {
        ...
        execute: function() {
          this.h=0;
          //измененные ячейки для синхронизации
          var changedCells = [];
          for (var y = 0; y < destH; y++){
            ...
            for (var x = 0; x < destW; x++){
              ...
              if (desty < this.maxDestY && destx < this.maxDestX ) {
                ...
                var to_column_name = _grid.getColumns()[destx].name,
                    to_cell_position = to_column_name + '' + (desty + 1);

                var processCell = _grid.getData().spreadsheet.getCell(to_cell_position);

                //var dt = _grid.getDataItem(desty);
                var oldValue = processCell.getCalculatedValue();
                this.oldValues[y][x] = { 'value': oldValue,
                                         'formula': processCell.formula || oldValue
                }; //dt[columns[destx]['id']];

                if (oneCellToMultiple)
                    _grid.getData().setCellData(processCell.getId(), clippedRange[0][0]);
                  //this.setDataItemValueForColumn(dt, columns[destx], clippedRange[0][0]);
                else
                    _grid.getData().setCellData(processCell.getId(), clippedRange[y] ? clippedRange[y][x] : '');

                changedCells.push(processCell);
          ...
          _on_self.onPasteCells.notify({ranges: [bRange]});
          ...
          for (var y = 0; y < destH; y++){
            for (var x = 0; x < destW; x++){
              ...
              if (desty < this.maxDestY && destx < this.maxDestX ) {
                ...
                var dt = _grid.getDataItem(desty);
                if (oneCellToMultiple)
                    _grid.getData().setCellData(processCell.getId(), this.oldValues[0][0].formula);
                  //this.setDataItemValueForColumn(dt, columns[destx], this.oldValues[0][0]);
                else
                    _grid.getData().setCellData(processCell.getId(), this.oldValues[y][x].formula);
      ...
      if (!_grid.getEditorLock().isActive() || _grid.getOptions().autoEdit) {
        ...
        if (e.which == keyCodes.V && (e.ctrlKey || e.metaKey)) {    // CTRL + V
           // e.preventDefault();
            ...
            //debugger;
            setTimeout(function(){
                _decodeTabularData(_grid, ta, _self);
            }, 100);
