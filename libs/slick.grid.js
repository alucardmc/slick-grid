...
(function ($) {
    ...
    function SlickGrid(container, data, columns, options) {
            ...
            if (!initialized) {
                ...
                if (!options.enableTextSelectionOnCells) {
                    ...
                    $viewport.bind("selectstart.ui", function (event) {
                        return $(event.target).is("input,textarea") || $(event.target).hasClass('editor-text');
                    });
                }
                ...
                if (navigator.userAgent.toLowerCase().match(/webkit/) &&
                    ...
                    $canvas.bind("mousewheel", handleMouseWheel2);
                }
            }
        }
        ...
        function setColumns(columnDefinitions, is_trigger) {
            ...
            is_trigger && trigger(self.onSetColumns, columns);
        }
        ...
        function appendCellHtml(stringArray, row, cell, colspan, item) {
            ...
            var newCell = "<div class='" + cellCss + " ";


            // if there is a corresponding row (if not, this is the Add New row or this data hasn't been loaded yet)
            if (item) {
                var value = getDataItemValueForColumn(item, m);
                var result_formatter = getFormatter(row, m)(row, cell, value, m, item),
                    result_css = result_formatter[1];
                stringArray.push(newCell);
                if(_.isArray(result_formatter)){
                    if(result_css.color_bg) stringArray.push(result_css.color_bg + " ");
                    if(result_css.color_font) stringArray.push(result_css.color_font);
                    stringArray.push("'>");
                    stringArray.push(typeof result_formatter[0] == 'undefined' ? '' : result_formatter[0]);
                } else {
                    stringArray.push("'>");
                    stringArray.push(typeof result_formatter == 'undefined' ? '' : result_formatter);
                }
            } else {
                stringArray.push(newCell + "'>");
            }
        ...
        function updateCell(row, cell) {
            ...
            if (currentEditor && activeRow === row && activeCell === cell) {
                ...
                var formatter_data = getFormatter(row, m)(row, cell, getDataItemValueForColumn(d, m), m, d);
                if(_.isArray(formatter_data)){
                    activeCellNode[0].innerHTML = typeof formatter_data[0] == 'undefined' ? '' : formatter_data[0];
                    if(formatter_data[1].color_bg) $(activeCellNode[0]).addClass(formatter_data[1].color_bg);
                    if(formatter_data[1].color_font) $(activeCellNode[0]).addClass(formatter_data[1].color_font);
                } else activeCellNode[0].innerHTML = typeof formatter_data == 'undefined' ? '' : formatter_data;
        ...
        function updateRow(row) {
            ...
            for (var columnIdx in cacheEntry.cellNodesByColumnIdx) {
                ...
                if (row === activeRow && columnIdx === activeCell && currentEditor) {
                    ...
                    var formatter = getFormatter(row, m);
                    var formatter_data = formatter(row, columnIdx, getDataItemValueForColumn(d, m), m, d);
                    if(_.isArray(formatter_data)){
                        node.innerHTML = typeof formatter_data[0] == 'undefined' ? '' : formatter_data[0];
                        if(formatter_data[1].color_bg) $(node).addClass(formatter_data[1].color_bg);
                        if(formatter_data[1].color_font) $(node).addClass(formatter_data[1].color_font);
                    } else node.innerHTML = typeof formatter_data == 'undefined' ? '' : formatter_data;
                }
            }
        ...
        function render(is_recreate_css) {
            if (!initialized) {
                return;
            }
            ...
            h_render = null;
            is_recreate_css && createCssRules();
        }
        ...
        function handleKeyDown(e) {
            ...
            if (!handled) {
                if (!e.shiftKey && !e.altKey && !e.ctrlKey) {
                    ...
                    } else if (_.contains([$.ui.keyCode.ENTER,  113], e.which)) { //TODO: added F2
                        if (options.editable) {
                            if (currentEditor) {
                                // adding new row
                ...
                } else if (e.which == $.ui.keyCode.TAB && e.shiftKey && !e.ctrlKey && !e.altKey) {
        ...
        function handleClick(e) {
            ...
            if(timer_click) clearTimeout(timer_click);
            if($('.editor-text').size()){
        ...
        function isCellPotentiallyEditable(row, cell) {
            ...
            // if cell is locked
            return !$(self.getCellNode(row, cell)).hasClass('lock_cell');
        }
        ...
        function makeActiveCellNormal() {
            ...
            if (activeCellNode) {
                ...
                if (d) {
                    ...
                    var formatter_data = formatter(activeRow, activeCell, getDataItemValueForColumn(d, column), column, d);
                    if(_.isArray(formatter_data)){
                        activeCellNode[0].innerHTML = typeof formatter_data[0] == 'undefined' ? '' : formatter_data[0];
                        if(formatter_data[1].color_bg) $(activeCellNode[0]).addClass(formatter_data[1].color_bg);
                        if(formatter_data[1].color_font) $(activeCellNode[0]).addClass(formatter_data[1].color_font);
                        ...
                        if (activeRow < getDataLength()) {
                            var editCommand = {
                                ...
                                undo: function () {
                                    ...
                                    updateRow(this.row);
                                    trigger(self.onCellChange, {
                                        //TODO: Fixed - when calling with not right cell position
                                        //                    row: activeRow,
                                        //                    cell: activeCell,
                                        row: this.row,
                                        cell: this.cell,
