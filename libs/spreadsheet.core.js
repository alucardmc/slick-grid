(function( Spreadsheet, $, undefined ) {
    ...
    function Sheet(name) {
        ...
        this.setCellData = function(pos,value, toInvalid){
            /** @param toInvalid - нужно обновлять строку **/
        ...
        this.getNextColumnName = function(grid){
            var name = this.getMaxColumn(grid).name,
                chars = name.split('');
            if(name.match(/^Z+$/)){
                name = name.replace(/Z/g, charArray[0]) + charArray[0];
                return name;
            }
            for(var i=chars.length-1; i>=0; i--){
                var ch = chars[i];
                if(ch == _.last(charArray)){
                    chars[i] = charArray[0];
                    if(chars.length==1){
                        chars[i+1] = charArray[0];
                        break;
                    }
                } else {
                    chars[i] = charArray[charArray.indexOf(ch)+1];
                    break;
                }
            }
            return chars.join('');
        };
        this.getMaxColumn = function(grid){
            var columns = _.filter(grid.getColumns(), function(c){
                return c.id != 'rowNum';
            });
            var max_length = _.max(columns, function(c){
                return c.id.length;
            });
            columns = _.filter(columns, function(c){
                return c.id.length == max_length.id.length;
            });
            var ids = _.pluck(columns, 'id');
            for(var i=0; i<max_length.id.length; i++){
                var ids_i = _.map(ids, function(id){ return indexArray[id[i]]; });
                var max_id_indx = _.max(ids_i);
                var max_id = charArray[max_id_indx];
                ids = _.filter(ids, function(id){ return id[i] == max_id });
            }
            return _.find(columns, function(c){ return c.id == ids[0]});
        };

        this.getFieldByColumnId = function(id){
            var add = 0,
                result = 0;
            id = id.split('').reverse().join('');
            for(var i=0; i<id.length; i++){
                add = i*26;
                result += add + indexArray[id[i]];
            }
            return result;
        }
    }
